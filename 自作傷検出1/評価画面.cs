...
namespace 自作傷検出1
{
    public partial class 評価画面 : Form
    {
        ...
        private void CannyとClose(int th1, int th2, int size, int 回数)
        {
            IplImage sample = 編集前の検査対象画像.Clone();
            IplConvKernel element = Cv.CreateStructuringElementEx(size, size, size / 2, size / 2, ElementShape.Rect);
            IplImage temp = sample.Clone();

            //Canny
            Cv.Canny(sample, sample,th1,th2);

            //Close
            Cv.MorphologyEx(sample, sample, temp, element, MorphologyOperation.Close, 回数);
            ...
            for (今の世代 = 1; 今の世代 <= 最終世代;今の世代++ )
            {
                ...
                if (今の世代 % 2 == 0 || 今の世代 == 1)//途中と最終結果を出力 
                    遺伝子情報をCSV出力(gene, 実験体数, パラメータ.Length / 2,
                      DateTime.Now.ToString("yy-MM-dd_") + 突然変異確率 + "-" + 突然変異の範囲 + "-" + 今の世代 + "_" + メイン画面.csvファイル名);
            }
        ...
        private int[,] 遺伝子を成績順にソート(int[,] 遺伝子, int 遺伝子の個数, int パラメータ数)
        {
            ...
            return p2をp3より大きくする(遺伝子,遺伝子の個数);
        }
        ...
        private int[,] p2をp3より大きくする(int[,] gene,int 遺伝子の個数)
        {
            ...
            while (今の世代 <= 最終世代)
            {
                ...
                if (今の世代 % 100 == 0 || 今の世代 == 1 || perfect)//途中と最終結果を出力 
                {
                    遺伝子情報をCSV出力(局所集団, 4, パラメータ.Length / 2,
                       DateTime.Now.ToString("yy-MM-dd_") + "Pf_" + 今の世代 + "_" + メイン画面.csvファイル名);
                }
                if (perfect) break;
                今の世代++;
            }
        ...
        private int[,] 次の局所集団を作成(int[,] group,int[,] パラメータ)
        {
            ...
            return p2をp3より大きくする(next_group, 4);
        }
        private void リストの作成()
        {
            int 累計達成者 = 0;
